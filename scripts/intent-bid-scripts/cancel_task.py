#!/usr/bin/env python3
"""
Aptos Bidding System - 取消任务脚本
Personal Agent 取消任务并获得全额退款
"""
...
async def cancel_task(
    ...
    try:
        ...
        print(f"任务取消成功! 交易版本: {tx_info['version']}")
        print("")
        print("任务已取消，处理结果：")
        print("- 全额资金已退还给任务创建者")
        print("- 任务状态更新为 CANCELLED")
        print("- 所有竞标记录已清除")
...
async def main():
    parser = argparse.ArgumentParser(description="取消任务并获得全额退款")
